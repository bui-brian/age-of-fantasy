...
func _on_unit_1_button_pressed() -> void:
	...
	var player_unit = player_spawner.spawn_unit(lm_scene, Unit.Faction.PLAYER, Vector2.RIGHT, Vector2(-200, 150))
	player_units.add_child(player_unit)
	
	var knight_scene = preload(knight_scene_path).instantiate()
	var enemy_unit = enemy_spawner.spawn_unit(knight_scene, Unit.Faction.ENEMY, Vector2.LEFT, Vector2(200, 150))
